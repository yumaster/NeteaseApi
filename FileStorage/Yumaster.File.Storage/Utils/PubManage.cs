...
namespace Yumaster.File.Storage
{
    public class PubManage
    {
            ...
            if (ID > 0)
            {
                ...
                if (Model.SharePasd == P2 || Model.ShareType == "0")//公开链接或者输入提取码正确
                {
                    string strSql = string.Format(@"SELECT share.CRUserName,share.RefType,share.ShareDueDate,share.CRDate,CASE WHEN share.RefType='file' then f.Name WHEN share.RefType='wj'  THEN folder.Name END Name 
                                            ...
                                            from File_Share share 
                                            LEFT join File_Info f on share.RefID=f.ID  and share.RefType='file'
                                            LEFT join  Folder_Info folder on share.RefID=folder.ID and share.RefType='wj' where share.ID={0} and share.IsDel!='Y'", ID);
                    DataTable dt = new File_ShareB().GetDTByCommand(strSql);
                    if (dt.Rows.Count > 0)
                    {
                        ...
                        {
                            msg.Result = dt;
                            msg.Result1 = AppSettingB.GetValueByKey("qyname");
                            msg.Result2 = AppSettingB.GetValueByKey("qyico");
                            msg.Result3 = AppSettingB.GetValueByKey("sysname");
                        }
                        else
                        {
                            msg.ErrorMsg = "分享已过期";
        ...
        /// <param name="P2"></param>
        /// <param name="UserInfo"></param>
        public void ISPUBLIC(JObject context, Msg_Result msg, string P1, string P2, Auth_UserB.UserInfo UserInfo)
        {
            int ID = int.Parse(P1);//
            File_Share Model = new File_ShareB().GetEntity(d => d.ID == ID && d.IsDel != "Y");
            msg.Result = "0";//默认公开分享
            if (Model != null)
            {
